...
public class CodeIgniteMITMusic extends AndroidViewComponent {
    ...
    private LinearLayout buildLibraryScreen(){ LinearLayout page=new LinearLayout(container.$context()); page.setOrientation(LinearLayout.VERTICAL); page.setPadding(dp(14), dp(8), dp(14), dp(8)); HorizontalScrollView tabScroll=new HorizontalScrollView(container.$context()); tabScroll.setHorizontalScrollBarEnabled(false); LinearLayout collectionTabs=new LinearLayout(container.$context()); collectionTabs.setOrientation(LinearLayout.HORIZONTAL); String[] tabs={"Library","Playlists","Favorites","Queue","History"}; for(final String tab:tabs){ TextView item=label(tab,14,true); item.setGravity(Gravity.CENTER); item.setPadding(dp(18),dp(10),dp(18),dp(10)); item.setBackground(round(cardColor,20)); item.setOnClickListener(new View.OnClickListener(){ public void onClick(View v){ openCollection(tab); }}); LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-2,-2); lp.setMargins(0,0,dp(8),dp(8)); collectionTabs.addView(item,lp); } tabScroll.addView(collectionTabs); dropdownList=new LinearLayout(container.$context()); dropdownList.setOrientation(LinearLayout.VERTICAL); searchDropdown=new ScrollView(container.$context()); searchDropdown.setFillViewport(false); searchDropdown.setBackground(round(cardColor,18)); searchDropdown.setElevation(dp(12)); searchDropdown.setVisibility(View.GONE); searchDropdown.addView(dropdownList); emptyStateText=label("No songs yet\nTap the update button or load a list from blocks.",16,false); emptyStateText.setGravity(Gravity.CENTER); ScrollView scroll=new ScrollView(container.$context()); songList=new LinearLayout(container.$context()); songList.setOrientation(LinearLayout.VERTICAL); scroll.addView(songList); page.addView(tabScroll,new LinearLayout.LayoutParams(-1,-2)); page.addView(emptyStateText,new LinearLayout.LayoutParams(-1,dp(140))); page.addView(scroll,new LinearLayout.LayoutParams(-1,0,1)); return page; }
    ...
    private void renderSongs(ArrayList<Song> data){
        ...
        for(int i=0;i<data.size();i++){
            ...
            ImageView art=new ImageView(container.$context()); art.setScaleType(ImageView.ScaleType.CENTER_CROP); art.setBackground(round(cardColor,12)); Drawable cover=loadDrawable(song.albumArt); if(cover!=null)art.setImageDrawable(cover); else art.setImageDrawable(loadDrawable(defaultAlbumArtPath));
    ...
    private void openCollection(String tab){
        currentScreen=tab;
        if(tab.equals("Library")){ emptyStateText.setText("No songs yet\nTap the update button or load a list from blocks."); renderSongs(songs); ScreenChanged(tab); return; }
    ...
    private void renderPlaylistNames(){ songList.removeAllViews(); emptyStateText.setVisibility(playlists.size()==0?View.VISIBLE:View.GONE); emptyStateText.setText("No playlists yet"); for(final String name:playlists.keySet()){ TextView row=titleLabel(name+"  •  "+playlists.get(name).size()+" songs",16); row.setPadding(dp(16),dp(18),dp(16),dp(18)); row.setBackground(round(cardColor,16)); row.setOnClickListener(new View.OnClickListener(){ public void onClick(View v){ showPlaylistDetail(name); }}); LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-1,-2); lp.setMargins(0,0,0,dp(10)); songList.addView(row,lp); } }
    private void showPlaylistDetail(final String name){ renderCollection(name,pathsToSongs(playlists.get(name))); PlaylistSelected(name); }
    ...
    @SimpleFunction(description="Adds a song to favorites.") public void AddToFavorites(String path){ favorites.add(path); FavoriteAdded(path); }
    @SimpleFunction(description="Removes a song from favorites.") public void RemoveFromFavorites(String path){ favorites.remove(path); FavoriteRemoved(path); }
    ...
    @SimpleFunction(description="Adds a song to the queue.") public void AddToQueue(String path){ queue.add(path); QueueUpdated(); }
    @SimpleFunction(description="Removes a song from the queue.") public void RemoveFromQueue(String path){ queue.remove(path); QueueUpdated(); }
