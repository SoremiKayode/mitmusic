...
public class CodeIgniteMITMusic extends AndroidViewComponent {
    ...
    private LinearLayout controlRow(String[] names){ LinearLayout r=new LinearLayout(container.$context()); r.setGravity(Gravity.CENTER); for(final String n:names){ TextView button=icon(n,new View.OnClickListener(){ public void onClick(View v){ control(n); }}); boolean main=n.equals("▶"); if(main)playPauseButton=button; button.setMinWidth(0); button.setMinHeight(0); LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(0,main?dp(56):dp(38),main?1.45f:1f); lp.setMargins(dp(1),0,dp(1),0); r.addView(button,lp); } return r; }
