...
public class CodeIgniteMITMusic extends AndroidViewComponent implements ActivityResultListener {
    ...
    @SimpleFunction(description="Imports and replaces favorites from a JSON song array.") public void ImportFavoritesJson(String json){ replacePaths(favorites,parseSongs(json)); refreshFavoriteRows(); }
    ...
    @SimpleFunction(description="Deletes all favorite records.") public void DeleteFavorites(){ favorites.clear(); refreshFavoriteRows(); }
    ...
    private void requestPermission(final String permission){ if(CheckPermission(permission)){ PermissionGranted(permission); return; } container.$form().runOnUiThread(new Runnable(){ public void run(){ container.$form().askPermission(permission,new PermissionResultHandler(){ public void HandlePermissionResponse(String p,boolean granted){ if(granted)PermissionGranted(p); else PermissionDenied(p); }}); }}); }
