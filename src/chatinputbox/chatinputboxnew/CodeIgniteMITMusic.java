...
public class CodeIgniteMITMusic extends AndroidViewComponent {
    ...
    @SimpleFunction(description="Shows dropdown.") public void ShowDropdown(){ dropdownList.setVisibility(View.VISIBLE); } @SimpleFunction(description="Hides dropdown.") public void HideDropdown(){ dropdownList.setVisibility(View.GONE); } @SimpleFunction(description="Updates dropdown. Required parameter: json (text): JSON text containing an array of song objects.") public void UpdateDropdown(String json){ dropdownList.removeAllViews(); try{ JSONArray a=new JSONArray(json); for(int i=0;i<a.length();i++){ JSONObject o=a.getJSONObject(i); final String path=o.optString("path"), title=o.optString("title"), artist=o.optString("artist"); TextView row=label("♪  "+title+"\n"+artist,14,false); row.setPadding(dp(14),dp(8),dp(14),dp(8)); row.setBackground(round(cardColor,16)); row.setOnClickListener(new View.OnClickListener(){ public void onClick(View v){ SearchItemClicked(path,title); }}); dropdownList.addView(row,new LinearLayout.LayoutParams(-1,-2)); }}catch(Exception e){} ShowDropdown(); }

    @SimpleFunction(description="Shows search bar.") public void ShowSearchBar(){ searchBar.setVisibility(View.VISIBLE); } @SimpleFunction(description="Hides search bar.") public void HideSearchBar(){ searchBar.setVisibility(View.GONE); } @SimpleFunction(description="Sets search hint. Required parameter: hint (text): Text displayed when the search field is empty.") public void SetSearchHint(String hint){ searchBar.setHint(hint); } @SimpleFunction(description="Clears search.") public void ClearSearch(){ searchBar.setText(""); }
    ...
    @SimpleFunction(description="Opens playlist drawer.") public void OpenPlaylistDrawer(){ playlistDrawer.setVisibility(View.VISIBLE); PlaylistOpened(); } @SimpleFunction(description="Closes playlist drawer.") public void ClosePlaylistDrawer(){ playlistDrawer.setVisibility(View.GONE); PlaylistClosed(); } @SimpleFunction(description="Opens sidebar.") public void OpenSidebar(){ sidebarDrawer.setVisibility(View.VISIBLE); DrawerOpened(); } @SimpleFunction(description="Closes sidebar.") public void CloseSidebar(){ sidebarDrawer.setVisibility(View.GONE); DrawerClosed(); } @SimpleFunction(description="Toggles sidebar.") public void ToggleSidebar(){ if(sidebarDrawer.getVisibility()==View.VISIBLE)CloseSidebar(); else OpenSidebar(); }
    ...
    private TextView icon(String t, View.OnClickListener l){ TextView b=label(t,22,true); b.setGravity(Gravity.CENTER); b.setMinWidth(dp(44)); b.setMinHeight(dp(44)); b.setPadding(dp(8),dp(6),dp(8),dp(6)); b.setBackground(round(Color.TRANSPARENT,24)); b.setContentDescription(t); b.setOnClickListener(l); registerIcon(t,b); return b; }
    ...
    private Drawable loadDrawable(String path){ if(path==null||path.length()==0)return null; String asset=path; if(asset.startsWith("file:///android_asset/"))asset=asset.substring(22); if(asset.startsWith("//"))asset=asset.substring(2); while(asset.startsWith("/"))asset=asset.substring(1); try{ return Drawable.createFromStream(container.$context().getAssets().open(asset),null); }catch(Exception e){} int slash=asset.lastIndexOf('/'); if(slash>=0){ try{ return Drawable.createFromStream(container.$context().getAssets().open(asset.substring(slash+1)),null); }catch(Exception e){} } try{ return Drawable.createFromPath(path); }catch(Exception e){} if(path.startsWith("file://")){ try{ return Drawable.createFromPath(path.substring(7)); }catch(Exception e){} } return null; }
