...
public class CodeIgniteMITMusic extends AndroidViewComponent {
    ...
    private void addSongRow(final Song song,final int index){
        ...
        if(removable){ TextView remove=actionButton("−",Color.rgb(95,95,95),"Remove from "+(visiblePlaylist.length()>0?"playlist":visibleCollection)); remove.setOnClickListener(new View.OnClickListener(){ public void onClick(View v){ removeVisibleSong(song.path); }}); actions.addView(remove,new LinearLayout.LayoutParams(dp(74),-1)); }
        TextView delete=actionButton("⌫",Color.rgb(190,45,45),"Delete song from phone"); delete.setOnClickListener(new View.OnClickListener(){ public void onClick(View v){ SongDeleteClicked(song.path); }}); actions.addView(delete,new LinearLayout.LayoutParams(dp(82),-1)); wrapper.addView(actions,new FrameLayout.LayoutParams(-1,-1));
        ...
        TextView share=icon("↗",new View.OnClickListener(){ public void onClick(View v){ ShareIconClicked(song.path); }}); TextView playlist=icon("＋",new View.OnClickListener(){ public void onClick(View v){ PlaylistIconClicked(song.path); ShowPlaylistPopup(song.path); }}); final boolean favorite=favorites.contains(song.path); TextView fav=icon(favorite?"⌫":"♡",new View.OnClickListener(){ public void onClick(View v){ if(favorites.contains(song.path))RemoveFromFavorites(song.path); else AddToFavorites(song.path); FavoriteClicked(song.path); refreshVisibleList(); }}); fav.setContentDescription(favorite?"Remove from favorites":"Add to favorites"); if(favorite)fav.setBackground(round(accentColor,22)); final boolean queued=queue.contains(song.path); TextView queueButton=icon(queued?"⌫":"≡",new View.OnClickListener(){ public void onClick(View v){ if(queue.contains(song.path))RemoveFromQueue(song.path); else AddToQueue(song.path); QueueClicked(song.path); refreshVisibleList(); }}); queueButton.setContentDescription(queued?"Remove from queue":"Add to queue"); if(queued)queueButton.setBackground(round(accentColor,22));
        LinearLayout.LayoutParams actionLp=new LinearLayout.LayoutParams(dp(44),dp(44)); actionLp.setMargins(dp(3),0,dp(3),0); row.addView(share,actionLp); row.addView(playlist,actionLp); row.addView(fav,actionLp); row.addView(queueButton,actionLp);
        ...
        installSwipe(row,dp(removable?156:82)); wrapper.addView(row,new FrameLayout.LayoutParams(-1,-2)); LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-1,-2); lp.setMargins(0,0,0,dp(12)); songList.addView(wrapper,lp);
    }
    ...
    private void installSwipe(final View foreground,final int revealWidth){ foreground.setOnTouchListener(new View.OnTouchListener(){ float downX; boolean moved; public boolean onTouch(View v,MotionEvent event){ if(event.getAction()==MotionEvent.ACTION_DOWN){ downX=event.getX(); moved=false; return true; } if(event.getAction()==MotionEvent.ACTION_MOVE){ float delta=event.getX()-downX; if(Math.abs(delta)>dp(8)||foreground.getTranslationX()<0){ moved=true; foreground.setTranslationX(Math.max(-revealWidth,Math.min(0,delta))); } return true; } if(event.getAction()==MotionEvent.ACTION_UP){ if(!moved)foreground.performClick(); foreground.animate().translationX(moved&&foreground.getTranslationX()<-revealWidth/3?-revealWidth:0).setDuration(animationSpeed).start(); return true; } if(event.getAction()==MotionEvent.ACTION_CANCEL){ foreground.animate().translationX(0).setDuration(animationSpeed).start(); return true; } return true; }}); }
    ...
    private void StopInternalPlayer(){ if(mediaPlayer!=null){ try{ mediaPlayer.stop(); }catch(Exception e){} mediaPlayer.release(); mediaPlayer=null; } setPlaying(false); sharePath=""; if(nowPlayingNav!=null)nowPlayingNav.setVisibility(View.GONE); if(nowPlayingScreen.getVisibility()==View.VISIBLE)OpenLibraryScreen(); }
    private void setPlaying(boolean playing){ isPlaying=playing; updatePlayPauseButton(); }
